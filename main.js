...
function parseDateFromBasename(basename, formats) {
...
function isPathIncluded(folder, scanFolders, excludeFolders) {
    ...
    for (const sc of scans) {
        if (!sc)
            return true; // 空字符串代表库根，即全库
        if (folder === sc || folder.startsWith(sc + "/"))
            return true;
    }
    return false;
}
/** 找出命中的分组；多个分组命中时，取路径更长（更具体）的那个 */
function matchGroupId(folder, groups) {
    let bestId = null;
    let bestLen = -1;
...
class DateNotesIndex {
    ...
    build(settings) {
        ...
        for (const file of this.app.vault.getMarkdownFiles()) {
            ...
            if (!isPathIncluded(folder, scanFolders, excludeFolders))
                continue;
            const date = parseDateFromBasename(file.basename, formats);
            if (!date)
                continue;
            const uid = getDateUID_1$1(date, "day");
            const entry = {
                file,
    ...
    buildIfSettingsChanged(settings) {
        const formats = (settings.dateFormats || [])
            .map(normalizeDateFormat)
            .filter(Boolean);
        const same = formats.length === this.formats.length &&
    ...
    /** 某个日期的全部笔记（已排序） */
    getNotes(date) {
        return this.notes[getDateUID_1$1(date, "day")] || [];
    }
    /** 某个日期有几篇 */
    getCount(date) {
    ...
    has(file) {
        if (!file)
            return false;
        return Object.values(this.notes).some((list) => list.some((n) => n.file.path === file.path));
    }
}
 ...
 */
const dateNotesIndex = new DateNotesIndex(window.app);
function createSelectedFileStore() {
    ...
    return Object.assign({ setFile: (file) => {
            ...
            if (date) {
                store.set(getDateUID_1$1(date, "day"));
                return;
            }
...
const streakSource = {
    getDailyMetadata: async (date) => {
        ...
        const showDots = get_store_value(settings).showNoteCount;
        const dots = showDots && count > 0
...
class CalendarView extends obsidian.ItemView {
    ...
    groupNameFor(groupId) {
        var _a;
        if (!groupId)
            return t.view.ungrouped;
        const group = (((_a = this.settings) === null || _a === void 0 ? void 0 : _a.groups) || []).find((g) => g.id === groupId);
        if (!group)
            return t.view.ungrouped;
    ...
    renderNotesFor(date) {
        ...
        const activePath = (_a = this.app.workspace.getActiveFile()) === null || _a === void 0 ? void 0 : _a.path;
    ...
    renderNoteItem(parent, note, activePath) {
    ...
    async createDateNote(date, inNewSplit) {
        var _a, _b;
        const folder = (((_a = this.settings) === null || _a === void 0 ? void 0 : _a.newNoteFolder) || "").trim();
        if (!folder) {
        ...
        const rawFormat = (((_b = this.settings) === null || _b === void 0 ? void 0 : _b.dateFormats) || [])[0] || "YYYY-MM-DD";
        const format = rawFormat.split("/").pop() || rawFormat;
        const filename = date.format(format);
        const dir = obsidian.normalizePath(folder);
        const path = obsidian.normalizePath(`${dir}/${filename}.md`);
        try {
            ...
            new obsidian.Notice(`Failed to create ${path}`);
        }
    }
}
...
class CalendarPlugin extends obsidian.Plugin {
    ...
    async onload() {
        ...
        this.addCommand({
            ...
            callback: () => {
                this.rebuildIndex();
                new obsidian.Notice(`Indexed ${this.indexedNoteCount()} date note(s).`);
            },
        });
